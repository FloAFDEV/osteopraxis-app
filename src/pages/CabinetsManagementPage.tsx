...
import { ArrowLeft, Building2, Edit, FileText, Mail, MapPin, Phone, Plus, Sparkles, Trash2 } from "lucide-react";
...
const CabinetsManagementPage = () => {
  ...
  const ImageWithLoader = ({
    ...
    return <div className={`w-full h-48 overflow-hidden flex justify-center items-center bg-gray-100 dark:bg-gray-700 ${className}`}>
				{loading && !error && <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>}
				{error && !loading && <p className="text-sm text-red-500">Image non disponible</p>}
				<img src={src} alt={alt} className={`w-full h-full object-cover ${loading || error ? "hidden" : "block"}`} onLoad={() => setLoading(false)} onError={() => {
        setLoading(false);
        setError(true);
      }} />
			</div>;
  ...
  return <Layout>
			<div className="min-h-screen bg-white dark:bg-gray-900">
				...
				<div className="bg-gray-400 dark:from-teal-700 dark:via-cyan-700 dark:to-blue-700 shadow-lg">
					<div className="container mx-auto px-6 py-8 bg-cyan-950">
						<div className="flex items-center gap-4 mb-6">
							<Button variant="outline" size="sm" onClick={() => navigate(-1)} className="bg-white/10 border-white/20 text-white hover:bg-white/20 hover:text-white">
				...
				<div className="container mx-auto px-6 py-8">
					{cabinets.length === 0 ? <div className="max-w-md mx-auto">
							...
							{cabinets.map(cabinet => <Card key={cabinet.id} className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-md">
									{cabinet.imageUrl && <div className="relative overflow-hidden">
											<ImageWithLoader src={cabinet.imageUrl} alt={cabinet.name} className="rounded-t-lg transition-transform duration-300 group-hover:scale-105" />
											<div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
										</div>}

									<CardHeader className="pb-4">
										<div className="flex items-start gap-4">
											{cabinet.logoUrl ? <div className="w-16 h-16 rounded-xl overflow-hidden shrink-0 shadow-md">
													<img src={cabinet.logoUrl} alt="Logo" className="w-full h-full object-contain bg-white" />
												</div> : <div className="w-16 h-16 bg-gradient-to-br from-teal-100 to-cyan-100 dark:from-teal-900 dark:to-cyan-900 rounded-xl flex items-center justify-center shrink-0 shadow-md">
											...
											<div className="flex-1 min-w-0">
												...
												{osteopathData[cabinet.osteopathId] && <p className="text-sm text-teal-600 dark:text-teal-400 font-medium mt-1">
														Dr.{" "}
									...
									<CardContent className="pt-0 space-y-4">
										<div className="space-y-3">
											...
											{cabinet.phone && <div className="flex items-center gap-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
											...
											{cabinet.email && <div className="flex items-center gap-3 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
										...
										{osteopathData[cabinet.osteopathId] && <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
												...
												<div className="flex flex-wrap gap-2">
													{osteopathData[cabinet.osteopathId].siret && <TooltipProvider>
															<Tooltip>
																<TooltipTrigger>
																	<Badge variant="outline" className="text-xs bg-teal-50 border-teal-200 text-teal-700 dark:bg-teal-900/20 dark:border-teal-800 dark:text-teal-300">
																		SIRET
																	</Badge>
																</TooltipTrigger>
																<TooltipContent>
													...
													{osteopathData[cabinet.osteopathId].adeli_number && <TooltipProvider>
															<Tooltip>
																<TooltipTrigger>
																	<Badge variant="outline" className="text-xs bg-cyan-50 border-cyan-200 text-cyan-700 dark:bg-cyan-900/20 dark:border-cyan-800 dark:text-cyan-300">
																		ADELI
																	</Badge>
																</TooltipTrigger>
																<TooltipContent>
													...
													{osteopathData[cabinet.osteopathId].ape_code && <TooltipProvider>
															<Tooltip>
																<TooltipTrigger>
									...
									<CardFooter className="bg-gray-50 dark:bg-gray-800 p-4 border-t border-gray-200 dark:border-gray-600 flex justify-between gap-3">
										<Button variant="outline" size="sm" asChild className="flex-1 hover:bg-teal-50 hover:border-teal-300 hover:text-teal-700 transition-colors">
											<Link to={`/cabinets/${cabinet.id}/edit`} className="flex items-center justify-center gap-2">
												<Edit className="h-4 w-4" />
												Modifier
											</Link>
										</Button>
										<Button variant="destructive" size="sm" className="flex-1 bg-red-500 hover:bg-red-600 transition-colors" onClick={() => confirmDelete(cabinet)}>
