...
import { ArrowLeft, Calendar as CalendarIcon, ChevronLeft, ChevronRight, Clock, FileText, Trash2, User, X } from "lucide-react";
			...
			<div className="flex flex-col p-4 sm:p-6 lg:p-8 mt-4">
				...
				{loading ? <div className="flex justify-center items-center py-12">
                  ...
                  locale: fr
								...
								<DaySchedule date={selectedDate} appointments={getDayAppointments(selectedDate)} getPatientById={getPatientById} onCancelAppointment={handleCancelAppointment} onDeleteAppointment={handleDeleteAppointment} actionInProgress={actionInProgress} />
							</div>
						</TabsContent>
                  ...
                  locale: fr
                    ...
                    return <Card key={appointment.id} className="hover-scale flex flex-col">
																...
																<CardContent className="p-3 flex-grow">
																	...
																	<Link to={`/appointments/${appointment.id}/edit`} className="block group mb-3" // Added bottom margin
                        >
																...
																<div className="flex flex-col sm:flex-row items-center justify-end gap-2 p-2 border-t bg-muted/30">
																	...
																	<AlertDialog>
																		<AlertDialogTrigger asChild>
																			<Button variant="ghost" size="sm" className="w-full sm:w-auto text-destructive hover:bg-destructive/10 h-8 px-3 flex items-center justify-center" // Responsive width, centered text/icon
                            disabled={isProcessingAction} title="Supprimer cette séance">
																		...
																		<AlertDialogContent>
																			...
																			<AlertDialogFooter>
																				...
																				<AlertDialogAction onClick={() => handleDeleteAppointment(appointment.id)} className="bg-destructive hover:bg-destructive/90" // Adjusted hover color
                              >
...
const DaySchedule = ({
  ...
  const getAppointmentForTimeSlot = (timeSlot: string) => {
    return appointments.find(appointment => format(parseISO(appointment.date), "HH:mm") === timeSlot);
  };
  return <div className="rounded-md border">
      ...
      return <div key={timeSlot} className={cn("flex border-b last:border-b-0 transition-colors", isCurrentTime ? "bg-primary/5" : "hover:bg-muted/50")}>
						...
						<div className="flex-1 p-3 min-w-0">
							...
							{appointment ? <div className="flex flex-col lg:flex-row items-start justify-between gap-2">
									{" "}
									...
									<div className="flex-grow min-w-0">
										...
										<div className="flex items-center gap-2 mb-1">
											...
											<Link to={`/patients/${appointment.patientId}`} className="font-medium hover:text-primary truncate" // Added truncate
                >
									...
									<div className="flex flex-wrap gap-2 justify-end w-full lg:w-auto shrink-0">
										...
										{/* Destructive Buttons */}
										<Button variant="ghost" size="sm" className="text-destructive hover:bg-destructive/10 flex-grow lg:flex-grow-0" // Responsive grow
              onClick={() => onCancelAppointment(appointment.id)} disabled={isProcessingAction || isCompleted} aria-label="Annuler cette séance">
