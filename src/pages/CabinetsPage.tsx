        ...
        {cabinets.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {cabinets.map(cabinet => (
              <Card key={cabinet.id} className="overflow-hidden bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700">
                ...
                <CardFooter className="bg-gray-50 dark:bg-gray-800/60 p-4 border-t dark:border-gray-700 flex justify-between">
                  <Button variant="outline" size="sm" asChild className="rounded-md">
                    <Link to={`/cabinets/${cabinet.id}`} className="flex items-center gap-1">
                      Détails
                    </Link>
                  </Button>
