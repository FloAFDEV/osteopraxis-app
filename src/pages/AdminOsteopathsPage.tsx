...
export default function AdminOsteopathsPage() {
	...
	const [selectedOsteopath, setSelectedOsteopath] =
		useState<OsteopathWithStatus | null>(null);
	...
	const handleActivateConfirm = async () => {
		...
		try {
			...
			toast.success(
				`✅ Ostéopathe ${selectedOsteopath.name} activé avec succès`,
			);
	...
	const handleUnblock = async (osteopath: OsteopathWithStatus) => {
		...
		try {
			...
			toast.success(
				`✅ Ostéopathe ${osteopath.name} débloqué (passé en mode demo)`,
			);
			loadData(); // Recharger les données
		} catch (error) {
					...
					<CardContent>
						...
						{demoStats && demoStats.avg_days_in_demo !== null && (
							<p className="text-sm text-muted-foreground mt-2">
								Moyenne :{" "}
								{Math.round(demoStats.avg_days_in_demo)} jours
								en démo
							</p>
						)}
					</CardContent>
						...
						<TableBody>
							{osteopaths.map((osteopath) => (
								<TableRow key={osteopath.id}>
									...
									<TableCell>
										{getStatusBadge(osteopath.status)}
									</TableCell>
									<TableCell>
										{osteopath.status === "demo" &&
											osteopath.daysInDemo !==
												undefined && (
			...
			>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							L'ostéopathe {selectedOsteopath?.name} passera en
							mode <strong>ACTIF</strong>. Les factures PDF
							générées ne seront plus marquées "DEMO".
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4 py-4">
						<div className="space-y-2">
							<Label htmlFor="activateReason">
								Raison de l'activation (optionnel)
							</Label>
							<Textarea
								id="activateReason"
								...
								onChange={(e) =>
									setActivateReason(e.target.value)
								}
								rows={3}
							/>
						</div>
			...
			<Dialog open={blockDialogOpen} onOpenChange={setBlockDialogOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							L'ostéopathe {selectedOsteopath?.name} sera{" "}
							<strong>BLOQUÉ</strong> et ne pourra plus accéder à
							l'application.
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4 py-4">
