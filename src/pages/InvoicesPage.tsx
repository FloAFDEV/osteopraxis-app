...
import { InvoiceRegistry } from "@/components/invoices/InvoiceRegistry";
...
import { exportInvoicesPeriod } from "@/utils/invoice-csv-export";
...
const InvoicesPage = () => {
	...
	const [osteopathDataMap, setOsteopathDataMap] = useState<Map<number, Osteopath>>(
		new Map()
	);
	const [selectedCabinetId, setSelectedCabinetId] = useState<number | "ALL" | null>(null);
	const [selectedOsteopathId, setSelectedOsteopathId] = useState<number | "ALL" | null>(
		null
	);
	const { osteopaths } = useOsteopaths();
	const { cabinets } = useCabinetsByOsteopath(
		selectedOsteopathId && selectedOsteopathId !== "ALL"
			? Number(selectedOsteopathId)
			: undefined
	);

	// Persist view mode
	useEffect(() => {
		localStorage.setItem("invoices-view-mode", viewMode);
	}, [viewMode]);
	...
	const invoicesFilteredByOsteoCabinet = useMemo(() => {
		...
		return invoices.filter((inv) => {
			const osteoOk =
				...
				(inv.osteopathId != null &&
					Number(inv.osteopathId) === Number(selectedOsteopathId));
			const cabinetOk =
				selectedCabinetId == null ||
				selectedCabinetId === "ALL" ||
				(inv.cabinetId != null &&
					Number(inv.cabinetId) === Number(selectedCabinetId));
	...
	// Invoices for the selected year (and optionally month)
	...
	}, [filteredInvoices, selectedYear, selectedMonth]);

	// Summary stats
	const summaryStats = useMemo(() => {
		const activeInvoices = displayedInvoices.filter(
			(inv) => inv.paymentStatus !== "CANCELED"
		);
		const paidInvoices = displayedInvoices.filter(
			(inv) => inv.paymentStatus === "PAID"
		);
		const pendingInvoices = displayedInvoices.filter(
			(inv) => inv.paymentStatus === "PENDING"
		);

		return {
			total: activeInvoices.reduce((sum, inv) => sum + inv.amount, 0),
	...
	useEffect(() => {
		const fetchRelatedData = async () => {
			...
			const osteopathIds = [
				...new Set(invoices.map((invoice) => invoice.osteopathId).filter(Boolean)),
			];
	...
	const loadInvoiceRelatedData = async (invoice: Invoice) => {
		try {
			...
			if (invoice.patientId) {
				...
				if (osteopathId) {
					osteopathData = await api.getOsteopathById(
						typeof osteopathId === "string" ? parseInt(osteopathId) : osteopathId
					);
					if (osteopathData?.id) {
	...
	const handleDownloadAllInvoices = async () => {
		try {
			...
			if (yearInvoices.length === 0) {
				toast.error(`Aucune note trouvée pour l'année ${selectedYear}`);
				return;
			}
			...
			toast.info(
				`Préparation du téléchargement des ${yearInvoices.length} notes de ${selectedYear}...`
			);
		} catch (error) {
	...
	const handleDownloadMonthInvoices = async (year: string, monthKey: string) => {
		...
		if (monthInvoices.length === 0) {
			...
			toast.error(`Aucune note trouvée pour ${monthLabel}`);
			return;
		}
		...
		toast.info(
			`Préparation du téléchargement des ${monthInvoices.length} notes de ${monthLabel}...`
		);
	};
				...
				<Layout>
					...
					<div className="mb-4 mt-6">
						...
						<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
							<div>
								<h1 className="text-lg font-semibold flex items-center gap-2">
									...
									<span className="text-slate-800 dark:text-slate-100">
										Registre des notes d'honoraires
									</span>
								</h1>
								{displayedInvoices.length > 0 && (
									<p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
										{selectedMonth
											...
											: selectedYear}{" "}
										· {summaryStats.count} notes ·{" "}
										{summaryStats.total.toLocaleString("fr-FR", {
										...
										{summaryStats.pendingCount > 0 && (
											<span className="text-amber-600 dark:text-amber-400">
												{" "}
												· {summaryStats.pendingCount} en attente
											</span>
										)}
									</p>
								)}
							...
							<div className="flex items-center gap-2">
								{/* View toggle */}
								<div className="flex items-center border border-gray-200 dark:border-gray-700 rounded-md p-0.5 bg-gray-50 dark:bg-gray-800">
									<button
										onClick={() => setViewMode("list")}
										className={cn(
											"p-1.5 rounded transition-colors",
											viewMode === "list"
												? "bg-white dark:bg-gray-700 shadow-sm text-slate-700 dark:text-slate-200"
						...
						<div className="mb-3">
							<Alert>
								<AlertTitle className="flex items-center gap-1.5">
									<Shield className="h-3.5 w-3.5" /> Données de facturation
									sensibles (locales)
								</AlertTitle>
								<AlertDescription>
									Les détails patients et factures restent stockés sur votre
									appareil. Partage via export sécurisé.
								</AlertDescription>
							</Alert>
						</div>
