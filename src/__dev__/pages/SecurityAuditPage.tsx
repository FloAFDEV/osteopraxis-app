...
const SECURITY_ISSUES: SecurityIssue[] = [
	{
		...
		description:
			"Les politiques RLS HDS_TOTAL_BLOCK_* bloquent tout accès aux données médicales, rendant l'application inutilisable.",
		impact: [
		...
		recommendation:
			"Remplacer immédiatement par des policies basées sur osteopathId",
		affectedTables: [
			"Patient",
			"Consultation",
			"MedicalDocument",
	...
	},
	{
		...
		description:
			'Policy "Enable all operations for authenticated users" avec expression TRUE permet à n\'importe quel utilisateur authentifié de tout faire.',
		impact: [
	...
	},
	{
		...
		description:
			"Les données de santé sensibles sont stockées en clair dans PostgreSQL.",
		impact: [
		...
		recommendation:
			"Activer pgcrypto et chiffrer colonnes: notes, diagnosis, medicalHistory",
	...
	},
	{
		...
		description:
			"Absence de validation au niveau base de données pour emails, SIRET, montants.",
		impact: [
		...
		recommendation:
			"Ajouter des constraints CHECK SQL et validation Zod côté client",
	...
	},
	{
		...
		description:
			"Jusqu'à 15 policies par table, beaucoup redondantes ou contradictoires.",
		impact: [
		...
		recommendation:
			"Simplifier et unifier les policies, supprimer les doublons",
	...
	},
	{
		...
		description:
			"Les données démo expirées ne sont pas supprimées automatiquement.",
		impact: [
		...
		recommendation:
			"Implémenter un job pg_cron de nettoyage toutes les 15 minutes",
	...
	},
	{
		...
		description:
			"Pas de trigger automatique pour logger tous les accès aux données sensibles.",
		impact: [
		...
		recommendation:
			"Ajouter triggers automatiques sur toutes tables avec données de santé",
...
export default function SecurityAuditPage() {
	...
	const criticalIssues = SECURITY_ISSUES.filter(
		(i) => i.severity === "critical",
	).length;
	const highIssues = SECURITY_ISSUES.filter(
		(i) => i.severity === "high",
	).length;
	const mediumIssues = SECURITY_ISSUES.filter(
		(i) => i.severity === "medium",
	).length;

	const totalScore = Object.values(SECURITY_SCORES).reduce(
		(sum, s) => sum + s.score,
		0,
	);
	const maxScore = Object.values(SECURITY_SCORES).reduce(
		(sum, s) => sum + s.max,
		0,
	);
		...
		<Layout>
			<div className="container mx-auto py-8 px-4 max-w-6xl">
				...
				<div className="mb-8">
					...
					{/* Score global */}
					<Alert
						className={`border-2 ${percentage < 70 ? "border-destructive bg-destructive/10" : "border-warning bg-warning/10"}`}
					>
						...
						<AlertDescription>
							<div className="mt-2 space-y-1">
								<p className="font-semibold">
									{criticalIssues} vulnérabilités critiques •{" "}
									{highIssues} élevées • {mediumIssues}{" "}
									moyennes
								</p>
								<p className="text-sm">
									Score insuffisant pour une application
									médicale manipulant des données de santé
									sensibles. Action immédiate requise.
								</p>
							</div>
						</AlertDescription>
					</Alert>
				...
				<Card className="mb-8">
					...
					<CardContent>
						<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
							{Object.entries(SECURITY_SCORES).map(
								([key, { score, max, label }]) => {
						...
						return (
							<Card key={issue.id} className="border-2">
								...
								<CardContent className="space-y-4">
									...
									{/* Tables affectées */}
									{issue.affectedTables &&
										issue.affectedTables.length > 0 && (
											<div>
												<p className="text-sm font-semibold mb-2">
													Tables affectées:
												</p>
												<div className="flex flex-wrap gap-2">
													{issue.affectedTables.map(
														(table) => (
															<Badge
																key={table}
																variant="outline"
				...
				<Card className="mt-8">
					<CardHeader>
						...
						<CardDescription>
							Checklist RGPD et HDS pour hébergement de données de
							santé
						</CardDescription>
					</CardHeader>
					<CardContent>
							...
							<div>
								...
								<div className="space-y-2">
									{[
										...
										{
											label: "Sauvegarde chiffrée",
											status: true,
										},
										{
											label: "Plan de reprise (PRA)",
											status: false,
										},
										{
				...
				<Card className="mt-8 bg-muted/30">
					...
					<CardContent>
						<div className="grid md:grid-cols-2 gap-3">
							{[
								{
									label: "Recommandations CNIL données de santé",
									url: "https://www.cnil.fr/fr/la-sante",
								},
								{
									label: "Référentiel HDS",
								...
								{
									label: "Supabase RLS Best Practices",
