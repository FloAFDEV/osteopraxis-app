...
const AdminTechDebugPage: React.FC = () => {
	...
	const validate = () => {
		...
		if (method === "pin" && !/^\d{4,8}$/.test(newCredential))
			return "Le PIN doit contenir 4 à 8 chiffres";
		if (method === "password" && newCredential.length < 8)
				...
				<div className="container mx-auto px-4 py-8">
					<Alert className="border-red-200 bg-red-50 dark:bg-red-950/20 dark:border-red-800">
						...
						<AlertDescription className="text-red-800 dark:text-red-200">
							...
							<p className="mt-2">
								Les outils de débogage technique ne sont pas
								disponibles en mode démonstration pour des
								raisons de sécurité.
							</p>
						</AlertDescription>
					</Alert>
						...
						<Card>
							<CardHeader>
								<CardTitle className="flex items-center gap-2">
									...
									Débloquer/modifier le code d’un praticien
								</CardTitle>
							</CardHeader>
							<CardContent className="space-y-4">
								<div>
									...
									<Input
										...
										placeholder="ex: praticien@exemple.com"
									/>
								</div>
								...
								<p className="text-sm text-muted-foreground">
									Le praticien pourra appliquer ce nouveau
									code depuis l’écran de déverrouillage.
								</p>
							</CardContent>
						</Card>
					</TabsContent>
