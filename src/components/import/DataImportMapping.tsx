...
export const DataImportMapping = ({
	data,
	onConfirm,
	onBack,
	...
	const [mapping, setMapping] = useState<ImportMapping>(() => {
		...
		data.headers.forEach((header) => {
			...
			if (
				...
				lowerHeader.includes("naissance") ||
				lowerHeader.includes("birth")
			) {
				targetField = "birthDate";
			} else if (
				lowerHeader.includes("genre") ||
				lowerHeader.includes("sexe") ||
				...
				lowerHeader.includes("profession") ||
				lowerHeader.includes("métier") ||
				...
				lowerHeader.includes("traitement") ||
				lowerHeader.includes("treatment")
			) {
				targetField = "currentTreatment";
			} else if (lowerHeader.includes("note")) {
				targetField = "notes";
			} else if (
				lowerHeader.includes("taille") ||
				lowerHeader.includes("height")
			) {
				targetField = "height";
			} else if (
				lowerHeader.includes("poids") ||
			...
			initialMapping[header] = {
				...
				isRequired: targetField
					? PATIENT_FIELDS[targetField].required
					: false,
				dataType: targetField
					? PATIENT_FIELDS[targetField].type
					: "text",
			};
		});
	...
	const handleMappingChange = (
		...
		setMapping((prev) => ({
			...
			[sourceColumn]: {
				...
				isRequired: targetField
					? PATIENT_FIELDS[targetField as PatientFieldKey].required
					: false,
				dataType: targetField
					? PATIENT_FIELDS[targetField as PatientFieldKey].type
					: "text",
			},
		}));
	};
	...
	const getAvailableFields = (currentField: string | null) => {
		...
		return Object.entries(PATIENT_FIELDS).filter(
			([key]) =>
				key === currentField ||
				!usedFields.includes(key as PatientFieldKey),
		);
	};
	...
	const getMissingRequiredFields = () => {
		...
		return Object.entries(PATIENT_FIELDS)
			.filter(
				([key, field]) =>
					field.required &&
					!mappedFields.includes(key as PatientFieldKey),
			)
			.map(([key]) => key);
	};
		...
		<div className="space-y-6">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Faites correspondre les colonnes de votre fichier avec
						les champs OstéoPraxis
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					{!canProceed && (
						<Alert variant="destructive">
							...
							<AlertDescription>
								Les champs obligatoires suivants doivent être
								mappés :{" "}
								{getMissingRequiredFields()
									.map(
										(field) =>
											PATIENT_FIELDS[
												field as PatientFieldKey
											].label,
									)
									.join(", ")}
							</AlertDescription>
					...
					<div className="p-4 bg-muted/50 rounded-lg">
						...
						<div className="grid grid-cols-2 gap-4 text-sm">
							...
							<div>
								...
								<span className="ml-2 font-medium">
									{Object.entries(PATIENT_FIELDS).filter(
