...
export function InvoiceExportButtons({
  ...
  const exportToExcel = async (year: string, month: string | null = null) => {
    try {
      ...
      const filteredInvoices = filterInvoicesByPeriod(invoices, year, month);
      
      if (filteredInvoices.length === 0) {
        const periodLabel = month 
          ? format(parseISO(`${year}-${month}-01`), 'MMMM yyyy', { locale: fr })
          : year;
      ...
      link.href = url;
      link.setAttribute('download', `Comptabilité_${periodLabel.replace(' ', '_')}.xlsx`);
      document.body.appendChild(link);
      link.click();
      link.parentNode?.removeChild(link);
      
      toast.success(`Export comptable pour ${periodLabel} généré avec succès`);
    } catch (error) {
      console.error("Erreur lors de l'export Excel:", error);
      toast.error("Une erreur est survenue lors de la génération de l'export comptable");
    }
  };
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" className="gap-2 bg-amber-50 hover:bg-amber-100 text-amber-700 border-amber-200">
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem onClick={handleExportCurrentMonth} className="gap-2">
          <Calendar className="h-4 w-4" />
          Exporter le mois sélectionné
        </DropdownMenuItem>
        <DropdownMenuItem onClick={handleExportFullYear} className="gap-2">
          <FileSpreadsheet className="h-4 w-4" />
