...
export function OsteopathProfileForm({
	...
	const form = useForm<z.infer<typeof osteopathSchema>>({
		...
		defaultValues: {
			...
			professional_title:
				currentOsteopath?.professional_title || "Ostéopathe D.O.",
	...
	const updateOsteopathProfile = async (
		data: z.infer<typeof osteopathSchema>,
	) => {
		try {
			...
			const updatedOsteopath = await api.updateOsteopath(
				osteopathId || currentOsteopath?.id,
				osteopathData,
			);
	...
	const createOsteopathProfile = async (
		data: z.infer<typeof osteopathSchema>,
	) => {
		try {
				...
				{isDemoMode && (
					<Card className="mt-6">
						<CardHeader>
							...
							<CardDescription>
								Ajoutez votre tampon pour qu'il apparaisse sur
								vos factures (mode démo - stockage local)
							</CardDescription>
						</CardHeader>
						<CardContent>
							{!stampPreview ? (
