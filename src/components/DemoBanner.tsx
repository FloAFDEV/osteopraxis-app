	...
	useEffect(() => {
		const checkDemoSession = async () => {
			try {
				...
				const { isDemoSession } =
					await import("@/utils/demo-detection");
					...
					if (session) {
						...
						const interval = setInterval(async () => {
							const currentSession =
								await DemoService.getCurrentDemoSession();
							if (currentSession) {
				...
				console.error(
					"Erreur lors de la vérification de la session démo:",
					error,
				);
			}
		};
			...
			<div className="container mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
				<div className="flex items-start md:items-center gap-3 flex-1">
					...
					<div className="flex-1 min-w-0">
						...
						<div className="text-sm md:text-sm text-blue-100 mt-1 break-words">
							{isTemporaryDemo ? (
								<>
									<span className="block md:inline">
										Session isolée •{" "}
										{sessionStats?.patientsCount || 0}{" "}
										patients
									</span>
