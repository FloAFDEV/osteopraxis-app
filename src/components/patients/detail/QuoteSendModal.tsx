...
import { Loader2, Mail, FileText, Calendar, Building, MapPin, Phone } from "lucide-react";
...
export function QuoteSendModal({ quote, isOpen, onClose, onSuccess }: QuoteSendModalProps) {
  ...
  const defaultSubject = `Devis ${quote.title} - ${quote.Patient ? `${quote.Patient.firstName} ${quote.Patient.lastName}` : ''}`;
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-gray-50 p-4 rounded-lg mb-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
            <div>
              <strong>Patient:</strong> {quote.Patient ? `${quote.Patient.firstName} ${quote.Patient.lastName}` : 'Non spécifié'}
            </div>
            <div>
            ...
            <div className="flex items-center gap-1">
              ...
              <strong>Valide jusqu'au:</strong> {format(new Date(quote.validUntil), 'dd/MM/yyyy', { locale: fr })}
            </div>
          </div>
          ...
          <div className="border-t pt-3 mt-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-medium text-sm">Informations légales incluses</span>
            </div>
            ...
            {loadingInfo ? (
              ...
              <div className="text-xs text-gray-600 space-y-1">
                {osteopathInfo && (
                  <>
                    <div className="flex items-center gap-1">
                      <span className="font-medium">{osteopathInfo.name}</span>
                      <span>-</span>
                      <span>{osteopathInfo.professional_title || "Ostéopathe D.O."}</span>
                    </div>
                    {cabinetInfo?.address && (
                      <div className="flex items-center gap-1">
                    ...
                    <div className="flex gap-4">
                      {osteopathInfo.rpps_number && <span>RPPS: {osteopathInfo.rpps_number}</span>}
                      {osteopathInfo.siret && <span>SIRET: {osteopathInfo.siret}</span>}
                    </div>
                    <p className="text-blue-600 font-medium">TVA non applicable – article 261-4-1° du CGI</p>
                  </>
                )}
              </div>
            )}
        ...
        <form onSubmit={handleSend} className="space-y-4">
          ...
          <div className="bg-blue-50 p-3 rounded-md text-sm text-blue-700">
            <strong>Note:</strong> Le devis sera automatiquement joint au format PDF à cet email avec toutes les mentions légales obligatoires.
          </div>
