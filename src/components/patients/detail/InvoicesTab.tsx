		...
		<div className="space-y-4 mt-6">
			...
			{sortedInvoices.length === 0 ? (
				...
				<div className="space-y-4">
					{Object.entries(invoicesByPeriod).map(([periodKey, periodInvoices]) => {
						const isExpanded = expandedPeriods[periodKey] !== false; // Par défaut, les périodes sont développées
						const periodLabel = format(parseISO(`${periodKey}-01`), 'MMMM yyyy', { locale: fr });
						
						return (
							<div key={periodKey} className="border rounded-lg overflow-hidden">
								<div 
									className="flex justify-between items-center bg-muted/50 p-3 cursor-pointer"
									onClick={() => togglePeriod(periodKey)}
								>
									<h4 className="font-medium capitalize">{periodLabel}</h4>
									<div className="flex items-center">
										<span className="text-sm text-muted-foreground mr-2">
											{periodInvoices.length} {periodInvoices.length > 1 ? 'notes' : 'note'}
										</span>
										{isExpanded ? (
											...
											<TableBody>
												{periodInvoices.map(invoice => (
													<TableRow key={invoice.id}>
														<TableCell>{formatAppointmentDate(invoice.date, 'dd MMMM yyyy')}</TableCell>
														<TableCell>{invoice.amount} €</TableCell>
														<TableCell>
															<div className={`inline-block px-2 py-1 rounded-full text-xs font-medium 
															${invoice.paymentStatus === 'PAID' ? 'bg-green-100 text-green-800' : 
															invoice.paymentStatus === 'PENDING' ? 'bg-amber-100 text-amber-800' : 
