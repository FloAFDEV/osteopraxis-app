...
import { Building, Calendar, Euro, FileText, Mail, MapPin, Phone, ScrollText, StickyNote, User } from "lucide-react";
				...
				<div className="space-y-6">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							...
							<div className="space-y-3">
								...
								{loading ? <div className="animate-pulse space-y-2">
										...
										{osteopathInfo?.rpps_number && <p>
												<strong>RPPS:</strong>{" "}
												{osteopathInfo.rpps_number}
											</p>}
										{osteopathInfo?.siret && <p>
												<strong>SIRET:</strong>{" "}
												{osteopathInfo.siret}
											</p>}
										{osteopathInfo?.ape_code && <p>
												<strong>Code APE:</strong>{" "}
												{osteopathInfo.ape_code}
											</p>}
										{!osteopathInfo?.rpps_number && !osteopathInfo?.siret && <p className="text-gray-500 italic">
													Numéros en cours
													d'attribution
												</p>}
									</div>}
                  ...
                  target.style.display = "none";
					...
					<div className="text-xs text-muted-foreground border-t pt-4">
						Dernière modification:{" "}
						{format(new Date(quote.updatedAt), "dd/MM/yyyy à HH:mm", {
            locale: fr
          })}
					</div>
				</div>
