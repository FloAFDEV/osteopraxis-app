				...
				<div className="space-y-4">
					{antecedentsItems.length === 0 ? (
						<span className="text-sm text-gray-500">
							Aucun antécédent important renseigné
						</span>
					) : (
							...
							return (
								...
								>
									<div className="flex items-center gap-2">
										{icon}
										<span
											className={`font-medium ${colorCls}`}
										>
											{item.label}
										</span>
										{importance &&
											importance.label === "Critique" && (
												<Badge
									...
									<span
										className={`text-foreground text-sm md:text-sm break-words whitespace-pre-line`}
									>
										{item.value}
									</span>
								</div>
