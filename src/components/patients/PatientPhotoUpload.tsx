...
export function PatientPhotoUpload({
	...
	const handleFileDrop = async (acceptedFiles: File[]) => {
		...
		try {
			...
			const errorMessage =
				err instanceof Error ? err.message : "Erreur upload photo";
		...
		return (
			<div className={`space-y-4 ${className}`}>
				<div className="flex items-center gap-4">
					<Avatar className="h-24 w-24 lg:h-32 lg:w-32">
						<AvatarFallback className="text-2xl lg:text-3xl bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-blue-900 dark:to-indigo-900">
							{initials || (
								<User className="h-12 w-12 lg:h-16 lg:w-16" />
							)}
						</AvatarFallback>
					</Avatar>
				...
				<Alert className="bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800">
					...
					<AlertDescription className="text-sm lg:text-sm text-amber-800 dark:text-amber-200">
						<strong>Stockage sécurisé non initialisé</strong> -
						Configurez le stockage HDS depuis les paramètres pour
						activer l'upload de photos.
					</AlertDescription>
				</Alert>
			</div>
		);
	...
	return (
		<div className={`space-y-4 ${className}`}>
			...
			<div className="flex items-center gap-4">
				...
				<div className="flex-1">
					...
					<p className="text-sm lg:text-sm text-muted-foreground">
						{photoUrl
							? isDemoMode
								? "Photo temporaire (mode démo)"
								: "Photo chiffrée localement"
							: "Aucune photo"}
					</p>
				</div>
							...
							<>
								...
								<div>
									...
									<p className="text-sm text-muted-foreground">
										Cliquez ou glissez une image (JPG, PNG,
										WEBP)
									</p>
			...
			{!error && (
				...
				>
					<AlertCircle
						className={`h-4 w-4 ${isDemoMode ? "text-purple-600 dark:text-purple-400" : "text-green-600 dark:text-green-400"}`}
					/>
					<AlertDescription
						className={`text-sm ${isDemoMode ? "text-purple-800 dark:text-purple-200" : "text-green-800 dark:text-green-200"}`}
					>
						{isDemoMode ? (
							<>
								<strong>Mode démonstration</strong> - Photo
								stockée temporairement dans votre navigateur (1
								heure). Les données seront effacées à la fin de
								la session démo.
							</>
						) : (
							<>
								<strong>100% local et sécurisé</strong> - Votre
								photo est chiffrée (AES-256-GCM) et stockée
								uniquement sur votre ordinateur. Aucune donnée
								envoyée au cloud.
							</>
						)}
					</AlertDescription>
