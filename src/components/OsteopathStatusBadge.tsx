...
export function OsteopathStatusBadge({
	className,
	showLabel = true,
}: OsteopathStatusBadgeProps) {
	const { user } = useAuth();
	...
	useEffect(() => {
		const fetchStatus = async () => {
			...
			try {
				const osteopath = await osteopathStatusService.getOsteopathById(
					user.osteopathId,
				);
				if (osteopath) {
			...
			<div className="flex items-start gap-3">
				...
				<div className="flex-1">
					...
					<p className="text-sm text-yellow-700 dark:text-yellow-400/90">
						Vous utilisez l'application en{" "}
						<strong>mode démo</strong>. Les factures PDF générées ne
						sont <strong>pas valables</strong> pour usage réel et
						portent un filigrane "DEMO".
					</p>
					<p className="text-sm text-yellow-600 dark:text-yellow-500 mt-2">
						Pour passer en mode actif et générer des factures
						officielles, contactez l'administrateur.
					</p>
				</div>
			</div>
		</div>
