...
export const LocalStorageSetup: React.FC<LocalStorageSetupProps> = ({
	...
	const [step, setStep] = useState<
		"location" | "security" | "verification" | "completion"
	>("location");
	...
	const testStorageSetup = async (config: LocalStorageConfig) => {
		...
		try {
			...
			throw new Error(
				`Échec du test de stockage: ${error instanceof Error ? error.message : "Erreur inconnue"}`,
			);
		}
	};
		...
		<div className="space-y-6">
			<div className="text-center space-y-4">
				...
				<div>
					...
					<p className="text-muted-foreground mt-2">
						Les données sensibles HDS seront stockées de manière
						sécurisée sur votre ordinateur
					</p>
				</div>
			</div>
			...
			{isValidating ? (
				...
				<div className="space-y-4">
					...
					>
						<AlertTriangle
							className={`h-4 w-4 ${storageStatus.available ? "text-green-600" : "text-red-600"}`}
						/>
						<AlertDescription
							className={
								storageStatus.available
		...
		<div className="space-y-6">
			<div className="text-center space-y-4">
				...
				<div>
					...
					<p className="text-muted-foreground mt-2">
						Choisissez votre méthode de protection des données
						sensibles
					</p>
				</div>
			</div>
			...
			<div className="space-y-4">
				<div>
					...
					<div className="grid grid-cols-2 gap-4 mt-2">
						...
						<div
							className={`p-4 border rounded-lg cursor-pointer transition-colors ${
								config.securityMethod === "password"
									? "border-primary bg-primary/5"
									: "border-border"
							}`}
							onClick={() =>
				...
				<div className="space-y-4">
					<div>
						...
						<Input
							...
							maxLength={
								config.securityMethod === "pin" ? 8 : undefined
							}
						/>
					</div>
					<div>
						<Label htmlFor="confirmCredential">Confirmation</Label>
						<Input
			...
			<div className="space-y-4">
				...
				<Alert className="border-blue-200 bg-blue-50">
					...
					<AlertDescription className="text-blue-800">
						<strong>Important :</strong> Assurez-vous de retenir
						votre{" "}
						{config.securityMethod === "pin"
							? "code PIN"
							: "mot de passe"}
						. Il sera nécessaire pour accéder à vos données à chaque
						session.
					</AlertDescription>
				</Alert>
			</div>
		...
		<div className="space-y-6 text-center">
			...
			<div className="bg-green-50 p-4 rounded-lg">
				<p className="text-sm text-green-800">
					Toutes les données sensibles HDS seront désormais stockées
					de manière sécurisée sur votre ordinateur avec un
					chiffrement AES-256.
				</p>
			</div>
		</div>
