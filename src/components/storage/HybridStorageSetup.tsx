...
import {
	hdsSecureManager,
	type HDSSecureConfig,
} from "@/services/hds-secure-storage/hds-secure-manager";
...
export function HybridStorageSetup() {
	...
	const handleSetup = async () => {
		...
		try {
			...
			toast.error(
				"Erreur lors de la configuration du stockage: " +
		...
		<div className="min-h-screen flex items-center justify-center bg-background p-4">
			<Card className="w-full max-w-md">
				<CardHeader className="text-center">
					...
					<CardDescription>
						Pour respecter la conformité HDS, vos données sensibles
						doivent être stockées localement de manière sécurisée.
					</CardDescription>
				</CardHeader>
				...
				<CardContent className="space-y-4">
					...
					<p className="text-sm text-muted-foreground text-center">
						Ce code sera requis à chaque connexion pour
						déverrouiller vos données sensibles.
					</p>
				</CardContent>
			</Card>
		</div>
