...
export function ROICalculator() {
	...
	const totalTimeGain =
		timeGainPatientManagement + timeGainAppointments + timeGainInvoicing;

	// Conversion en rendez-vous supplémentaires possibles
	const averageSessionDuration = 1; // 1h par séance
	const additionalSessions = Math.floor(
		totalTimeGain / averageSessionDuration,
	);
	...
	const proPlanAdditionalSessions = Math.floor(
		proPlanTimeGain / averageSessionDuration,
	);
	const proPlanAdditionalRevenue =
		proPlanAdditionalSessions * pricePerSession;
		...
		<div className="space-y-6">
			...
			<Card className="border-2">
				<CardHeader>
					...
					<CardDescription>
						Ajustez les paramètres selon votre activité pour
					...
					<CardContent className="pt-6 space-y-4">
						<div className="space-y-3">
							<div className="flex justify-between text-sm">
								...
								<span className="font-semibold text-foreground">
									+{proPlanAdditionalRevenue}€
								</span>
							</div>
							<div className="flex justify-between text-sm">
								<span className="text-muted-foreground">
									Optimisation par analytics
								</span>
						...
						<div className="p-3 border rounded-lg bg-primary/5">
							...
							<p className="text-sm text-muted-foreground">
								Inclut gestion d'équipe et pilotage par la
								donnée
							</p>
						</div>
					</CardContent>
			...
			<Card className="border bg-muted/20">
				<CardContent className="p-4">
					<p className="text-sm text-muted-foreground">
						<strong>Méthodologie de calcul :</strong> Les gains de
						temps sont basés sur des moyennes observées : 6 minutes
						économisées par patient pour la gestion administrative,
						5 minutes par rendez-vous pour la planification, et 3
						minutes par note d'honoraires pour la facturation. Le
						plan Pro intègre des optimisations supplémentaires
						(+30%) grâce aux analytics et aux fonctionnalités de
						pilotage avancées.
					</p>
				</CardContent>
			</Card>
