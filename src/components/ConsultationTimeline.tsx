...
export function ConsultationTimeline({ events, onViewCR, className }: ConsultationTimelineProps) {
  if (events.length === 0) {
    return (
      <Card className={cn('p-12 text-center', className)}>
        <Calendar className="h-12 w-12 text-muted-foreground mx-auto mb-4 opacity-50" />
        <p className="text-muted-foreground">Aucune consultation pour l'instant</p>
      </Card>
    );
  }
      ...
      {events.map((event, index) => {
        ...
        return (
          <div key={event.id} className="relative flex gap-4">
            {/* Timeline bar */}
            {!isLast && (
              <div className="absolute left-5 top-12 w-0.5 h-full bg-border" aria-hidden="true" />
            )}
            ...
            <div className="relative flex-shrink-0">
              <div
                className={cn(
                  ...
                  event.status === 'completed' && 'bg-green-50 dark:bg-green-950/30',
                  event.status === 'upcoming' && 'bg-blue-50 dark:bg-blue-950/30',
                  event.status === 'cancelled' && 'bg-gray-50 dark:bg-gray-950/30'
                )}
              >
                  ...
                  <div className="bg-muted/30 rounded-lg p-3 space-y-2">
                    ...
                    {event.cr.chiefComplaint && (
                      <div className="text-sm">
                        ...
                        <span className="text-muted-foreground">{event.cr.chiefComplaint}</span>
                      </div>
                    )}
                    ...
                    {event.cr.diagnosis && (
                      <div className="text-sm">
                        ...
                        <span className="text-muted-foreground">{event.cr.diagnosis}</span>
                      </div>
                    )}

                    {/* Techniques utilisées */}
                    {event.cr.techniquesUsed && event.cr.techniquesUsed.length > 0 && (
                      <div className="text-sm">
                        <span className="font-medium text-foreground block mb-1">
                          Techniques :
                        </span>
                        <div className="flex flex-wrap gap-1">
                          {event.cr.techniquesUsed.slice(0, 3).map((technique, i) => (
                            <Badge key={i} variant="outline" className="text-xs">
                              {technique}
                            </Badge>
                          ))}
                          {event.cr.techniquesUsed.length > 3 && (
...
export function CompactTimeline({ events, maxItems = 5, className }: CompactTimelineProps) {
