...
export function SystemHealthPanel() {
	...
	const getStatusBadge = (status: string) => {
		...
		return (
			<Badge
				className={`${colors[status as keyof typeof colors] || colors.healthy} border`}
			>
			...
			<Card>
				...
				<CardContent>
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						<Button
							variant="outline"
							className="h-auto p-4 justify-start hover:bg-accent/50 transition-colors"
						>
						...
						<h3 className="text-lg font-semibold">
							{healthScore >= 90
								...
								: healthScore >= 70
									...
									: healthScore >= 50
										? "Système nécessite une attention"
										: "Système critique"}
						</h3>
						<p className="text-muted-foreground">
							{
								metrics.filter((m) => m.status === "healthy")
									.length
							}{" "}
							sur {metrics.length} métriques sont saines
						</p>
					</div>
				</CardContent>
			</Card>
