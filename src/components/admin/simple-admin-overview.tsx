...
export function SimpleAdminOverview() {
  ...
  const loadSystemOverview = async () => {
    try {
      ...
      const { data: stats, error } = await supabase.rpc('admin_get_system_stats');
  ...
  const runQuickAction = async (action: string) => {
    ...
    try {
      switch (action) {
        case 'cleanup':
          const { data: cleanupResult, error: cleanupError } = await supabase.rpc('admin_cleanup_old_logs', { days_old: 30 });
          if (cleanupError) throw cleanupError;
          toast.success(`Nettoyage terminé: ${(cleanupResult as any)?.deleted_count || 0} logs supprimés`);
          break;
          
        case 'optimize':
          const { data: optimizeResult, error: optimizeError } = await supabase.rpc('admin_optimize_performance');
  ...
  const getHealthBadge = (health: string) => {
    switch (health) {
      case 'healthy':
        return <Badge className="bg-green-100 text-green-800"><CheckCircle className="h-3 w-3 mr-1" />Système OK</Badge>;
      case 'warning':
        return <Badge className="bg-yellow-100 text-yellow-800"><AlertTriangle className="h-3 w-3 mr-1" />Attention</Badge>;
      case 'critical':
        return <Badge className="bg-red-100 text-red-800"><AlertTriangle className="h-3 w-3 mr-1" />Critique</Badge>;
      default:
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{overview.activeUsers} actifs</p>
              </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Cabinets</p>
                <p className="text-2xl font-bold">{overview.totalCabinets}</p>
              </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <Badge variant="outline">{overview.storageUsed}</Badge>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm">Dernier backup</span>
              <Badge variant="outline">{overview.lastBackup}</Badge>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            >
              ...
              {actionLoading === 'cleanup' ? 'Nettoyage...' : 'Nettoyer les logs'}
            </Button>
            ...
            >
              ...
              {actionLoading === 'refresh' ? 'Actualisation...' : 'Actualiser les données'}
            </Button>
          </CardContent>
        </Card>
      </div>
