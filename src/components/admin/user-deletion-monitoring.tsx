...
export function UserDeletionMonitoring() {
	...
	const loadDeletionData = async () => {
		try {
			...
			const { data: deletedUsersData, error: deletedError } =
				await supabase
					.from("User")
					.select("*")
			...
			const deletedThisWeek =
				deletedUsersData?.filter(
					(user) => new Date(user.deleted_at) >= oneWeekAgo,
				).length || 0;

			const deletedThisMonth =
				deletedUsersData?.filter(
					(user) => new Date(user.deleted_at) >= oneMonthAgo,
				).length || 0;

			// Calcul de la rétention moyenne (simulation)
			const averageRetention =
				deletedUsersData?.reduce((sum, user) => {
					const createdAt = new Date(user.created_at);
					const deletedAt = new Date(user.deleted_at);
					const diffDays = Math.floor(
						(deletedAt.getTime() - createdAt.getTime()) /
							(1000 * 60 * 60 * 24),
					);
					return sum + diffDays;
				}, 0) || 0;

			const averageRetentionDays = deletedUsersData?.length
				? averageRetention / deletedUsersData.length
				: 0;
			...
			console.error(
				"Erreur lors du chargement des données de suppression:",
				error,
			);
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
				...
				<Card>
					...
					<CardContent>
						<div className="flex justify-between items-center">
							<div className="text-2xl font-bold">
								{Math.round(stats.averageRetentionDays)}j
							</div>
			...
			<Tabs defaultValue="recent" className="w-full">
				...
				<TabsContent value="recent">
					<Card>
						<CardHeader>
							<CardTitle>
								Utilisateurs Supprimés Récemment
							</CardTitle>
						</CardHeader>
						<CardContent>
				...
				<TabsContent value="patterns">
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						<Card>
							<CardHeader>
								<CardTitle>
									Principales Raisons de Suppression
								</CardTitle>
							</CardHeader>
							<CardContent>
								<div className="space-y-3">
									{stats.topDeletionReasons.map(
										(reason, index) => (
											<div
												key={index}
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
								<div className="p-4 border rounded-lg">
									...
									<p className="text-sm text-muted-foreground mb-3">
										Identifier et réengager les utilisateurs
										inactifs avant suppression
									</p>
								...
								<div className="p-4 border rounded-lg">
									...
									<p className="text-sm text-muted-foreground mb-3">
										Envoyer des emails personnalisés aux
