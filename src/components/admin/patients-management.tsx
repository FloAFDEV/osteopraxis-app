...
export function PatientsManagement() {
	...
	const [selectedPatient, setSelectedPatient] =
		useState<AdminPatientSearchResult | null>(null);
	...
	const handleSearch = async () => {
		try {
			...
			const { data } = await adminApiService.getPatients(
				...
				undefined,
				selectedCabinet && selectedCabinet !== "all"
					? parseInt(selectedCabinet)
					: undefined,
			);
			setSearchResults(data);
		} catch (error) {
	...
	const loadOrphans = async () => {
		try {
			...
			console.error(
				"Erreur lors du chargement des patients orphelins:",
				error,
			);
	...
	const PatientCard = ({
		...
		patient: AdminPatientSearchResult;
	}) => (
		<Card className="hover:shadow-md transition-shadow">
			<CardContent className="p-4">
				...
				<div className="mt-3 text-sm text-muted-foreground">
					Créé le {formatDate(patient.created_at)} • Modifié le{" "}
								...
								<div className="space-y-6">
									{Object.entries(groupedDuplicates).map(
										([groupId, duplicateGroup]) => (
											<Card
												key={groupId}
												className="border-orange-200 bg-orange-50/50 dark:bg-orange-900/10"
											>
						...
						<CardContent>
							{orphans.length === 0 ? (
								<div className="text-center py-8">
									...
									<p className="text-muted-foreground">
										Tous les patients ont un ostéopathe
										assigné.
									</p>
								</div>
							) : (
								<div className="grid gap-4">
									{orphans.map((patient) => (
										<Card
											key={patient.id}
											className="border-red-200 bg-red-50/50 dark:bg-red-900/10"
										>
											<CardContent className="p-4">
												<div className="flex justify-between items-start mb-3">
													<div>
								...
								<div>
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
										<div>
											...
											<p>
												{selectedPatient.osteopath_name}
											</p>
										</div>
										<div>
											<label className="text-sm font-medium text-muted-foreground">
