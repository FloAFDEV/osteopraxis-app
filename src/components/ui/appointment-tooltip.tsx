...
export function AppointmentTooltip({
	...
	const getGenderIcon = (gender?: string | null) => {
		if (gender === "Homme" || gender === "MALE") {
			return (
				<UserIcon className="h-3 w-3 text-blue-500 dark:text-blue-400 flex-shrink-0" />
			);
		} else if (gender === "Femme" || gender === "FEMALE") {
			return (
				<UserIcon className="h-3 w-3 text-pink-500 dark:text-pink-400 flex-shrink-0" />
			);
		}
		return (
					...
					<div className="space-y-2">
						...
						<div className="space-y-2 max-h-32 overflow-y-auto">
										...
										>
											<div className="flex items-center gap-2 min-w-0 flex-1">
												...
												<div className="min-w-0 flex-1">
													...
													<p className="text-sm truncate text-gray-600 dark:text-gray-300 opacity-90">
														{appointment.reason}
													</p>
												</div>
											</div>
							...
							{appointments.length > 5 && (
								<div className="text-center text-sm text-gray-500 dark:text-gray-300 pt-1 border-t border-gray-200 dark:border-gray-600">
									+{appointments.length - 5} autres
									rendez-vous
								</div>
							)}
						</div>
