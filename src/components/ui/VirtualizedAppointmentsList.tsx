 ...
 */
export const VirtualizedAppointmentsList: React.FC<
	VirtualizedAppointmentsListProps
> = ({
	appointments,
	patients,
	onStatusChange,
	...
	const { visibleItems, totalHeight, scrollTop, setScrollTop } =
		useVirtualization(
			appointments,
			itemHeight,
			containerHeight,
		...
		<div className={className}>
			...
			>
				<div style={{ height: totalHeight, position: "relative" }}>
					{visibleItems.map(
						({ item: appointment, index, offsetTop }) => (
							<div
			...
			<div className="text-sm text-muted-foreground mt-2 text-center">
				Affichage virtualisé: {visibleItems.length} sur{" "}
				{appointments.length} éléments
			</div>
		</div>
	);
};
