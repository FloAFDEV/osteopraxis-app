	...
	const form = useForm<ConsultationReportFormValues>({
		...
		defaultValues: {
			...
			historyOfPresentIllness:
				existingReport?.historyOfPresentIllness || "",
			...
			nextAppointmentSuggested:
				existingReport?.nextAppointmentSuggested || "",
		},
	});
	...
	const onSubmit = async (data: ConsultationReportFormValues) => {
		...
		try {
			const reportData: CreateConsultationReportPayload = {
				...
				testsPerformed:
					data.testsPerformed && data.testsPerformed.length > 0
						? data.testsPerformed
						: null,
				diagnosis: data.diagnosis || null,
				techniquesUsed:
					selectedTechniques.length > 0 ? selectedTechniques : null,
			...
			if (existingReport) {
				// Mise à jour
				savedReport =
					await hdsSecureConsultationReportService.updateConsultationReport(
						existingReport.id,
						reportData,
					);
				...
				// Création
				savedReport =
					await hdsSecureConsultationReportService.createConsultationReport(
						reportData,
					);
		...
		<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
				...
				<CardHeader>
					<CardTitle className="flex items-center gap-2">
						...
						{existingReport
							...
							: "Nouveau compte-rendu ostéopathique"}
					</CardTitle>
					<p className="text-sm text-muted-foreground">
						🔐 Les données sont stockées localement et chiffrées
						(AES-256-GCM)
					</p>
				</CardHeader>
			</Card>
				...
				<CardContent className="space-y-6">
					...
					<div className="space-y-2">
						<Label htmlFor="treatmentNotes">
							Notes détaillées du traitement
						</Label>
						<Textarea
							id="treatmentNotes"
			...
			<Card>
				...
				<CardContent className="space-y-4">
					...
					<div className="space-y-2">
						<Label htmlFor="nextAppointmentSuggested">
							Date suggérée du prochain rendez-vous
						</Label>
						<Input
