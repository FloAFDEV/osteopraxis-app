...
export const GoogleEventCard = memo(
	({ event, compact = false }: { event: any; compact?: boolean }) => {
		...
		if (compact) {
			return (
				<div
					className={cn(
						"p-1 rounded text-sm truncate transition-colors hover:bg-primary/10",
		...
		return (
			<Card className="hover-scale flex flex-col border-l-4 border-l-blue-500 bg-blue-50/50">
					...
					<div className="flex items-center justify-between mb-2">
						...
						<div className="flex gap-1">
							{event.is_doctolib && (
								<Badge
									variant="outline"
									className="text-green-700 border-green-300 text-sm"
								>
									Doctolib
								</Badge>
							)}
							<Badge
								variant="outline"
								className="text-blue-700 border-blue-300 text-sm"
							>
								Google
							</Badge>
						</div>
				...
				<div className="px-3 pb-2 flex justify-between items-center">
					<p className="text-sm text-blue-600 bg-blue-100 px-2 py-1 rounded">
			...
			<Card className="hover-scale flex flex-col">
				<CardContent className="p-3 flex-grow">
					<div className="flex items-center justify-between mb-2">
						...
						{isCompleted && (
							<Badge className="bg-amber-500">Terminé</Badge>
						)}
					</div>
					<Link
						to={`/appointments/${appointment.id}/edit`}
