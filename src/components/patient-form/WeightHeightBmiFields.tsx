...
export const WeightHeightBmiFields = ({ form }: WeightHeightBmiFieldsProps) => {
	...
	const getBmiBackgroundColor = (bmi: number | null): string => {
		...
		if (bmi < 18.5)
			return "bg-blue-100 dark:bg-blue-900 text-black dark:text-white"; // Sous la normale
		if (bmi >= 18.5 && bmi <= 24.9)
			return "bg-green-100 dark:bg-green-900 text-black dark:text-white"; // Normale
		if (bmi >= 25 && bmi <= 29.9)
			return "bg-yellow-100 dark:bg-yellow-800 text-black dark:text-white"; // Surpoids
		if (bmi >= 30)
			return "bg-red-100 dark:bg-red-900 text-black dark:text-white"; // Obèse
