...
export function CabinetAssociationManagement() {
	...
	const loadCabinets = async () => {
		...
		try {
			...
			const associatedCabinetIds = await api.getOsteopathCabinets(
				user.osteopathId,
			);
			...
			const validAssociatedCabs = associatedCabsData.filter(
				Boolean,
			) as Cabinet[];
			...
			console.error(
				"CabinetAssociationManagement: Erreur lors du chargement des cabinets:",
				error,
			);
	...
	const handleDissociateCabinet = async (cabinetId: number) => {
		...
		try {
			...
			await api.dissociateOsteopathFromCabinet(
				user.osteopathId,
				cabinetId,
			);
			toast.success("Cabinet dissocié avec succès", {
				description:
					"Vous n'avez plus accès aux patients de ce cabinet.",
			});
			await loadCabinets();
		} catch (error) {
			console.error(
				"CabinetAssociationManagement: Erreur lors de la dissociation:",
				error,
			);
			toast.error("Erreur lors de la dissociation", {
				description:
					"Impossible de dissocier le cabinet. Veuillez réessayer.",
			});
		}
	};
	...
	const ownedCabinets = associatedCabinets.filter(
		(cabinet) => cabinet.osteopathId === user?.osteopathId,
	);
	const invitedCabinets = associatedCabinets.filter(
		(cabinet) => cabinet.osteopathId !== user?.osteopathId,
	);
		...
		<div className="space-y-6">
			<div className="flex justify-between items-start">
				<div>
					<div className="flex items-center gap-2">
						...
						<HelpButton content="Gérez vos cabinets : ceux que vous possédez et ceux auxquels vous êtes invité. Le système d'invitations sécurisé permet une collaboration professionnelle contrôlée." />
					</div>
					<p className="text-sm text-muted-foreground mt-1">
						Gérez les cabinets auxquels vous êtes associé (sécurisé
						par invitations)
					</p>
				</div>
			...
			<Alert className="border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-900/20">
				...
				<AlertDescription className="text-blue-800 dark:text-blue-300">
					<strong>Modèle SaaS :</strong> Pour des raisons de sécurité
					et de conformité, seuls les propriétaires de cabinets
					peuvent générer des invitations. Contactez le propriétaire
					du cabinet que vous souhaitez rejoindre pour obtenir un code
					d'invitation.
				</AlertDescription>
			</Alert>
								...
								<div className="flex items-center gap-3">
									...
									<div>
										<div className="flex items-center gap-2">
											...
											<Badge
												variant="outline"
												className="text-sm bg-green-100 text-green-800 border-green-300 dark:bg-green-900/30 dark:text-green-300 dark:border-green-700"
											>
												Propriétaire
											</Badge>
										</div>
			...
			{invitedCabinets.length > 0 && (
				<div className="space-y-3">
					...
					{invitedCabinets.map((cabinet) => (
						<div
							key={`invited-${cabinet.id}`}
							className="flex items-center justify-between p-4 border rounded-lg bg-blue-50/50 dark:bg-blue-900/10 border-blue-200 dark:border-blue-800"
						>
							<div className="flex-1">
								<div className="flex items-center gap-3">
									...
									<div>
										<div className="flex items-center gap-2">
											...
											<Badge
												variant="outline"
												className="text-sm bg-blue-100 text-blue-800 border-blue-300 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-700"
											>
												Invité
											</Badge>
										</div>
			...
			{associatedCabinets.length === 0 && (
				<div className="text-center py-12 bg-muted/50 rounded-lg border-2 border-dashed">
					...
					<p className="text-muted-foreground mb-6 max-w-md mx-auto">
						Créez votre propre cabinet ou demandez à un propriétaire
						de cabinet de vous envoyer un code d'invitation pour
						rejoindre son équipe et collaborer en toute sécurité.
					</p>
