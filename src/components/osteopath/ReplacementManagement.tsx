...
export function ReplacementManagement() {
  const { replacements, loading, createReplacement, updateReplacement, deleteReplacement, toggleReplacement } = useOsteopathReplacements();
  ...
  const availableReplacements = osteopaths.filter(o => o.access_type !== 'self');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const currentOsteopath = osteopaths.find(o => o.access_type === 'self');
      if (!currentOsteopath) {
      ...
      const replacementData = {
        ...
        replacement_osteopath_id: Number(formData.replacement_osteopath_id),
  ...
  const handleEdit = (replacement: any) => {
    ...
    setFormData({
      replacement_osteopath_id: String(replacement.replacement_osteopath_id),
  ...
  const handleToggleActive = async (id: number, currentStatus: boolean) => {
    try {
      ...
      toast.success(currentStatus ? "Remplacement désactivé" : "Remplacement activé");
    } catch (error: any) {
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <p className="text-sm text-muted-foreground">
              Gérez qui peut vous remplacer et créer des factures en votre nom
            </p>
            <Dialog open={isDialogOpen} onOpenChange={(open) => {
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>
                    {editingReplacement ? "Modifier le remplacement" : "Ajouter un remplaçant"}
                  </DialogTitle>
                </DialogHeader>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <Label htmlFor="replacement_osteopath">Ostéopathe remplaçant</Label>
                    <Select
                      value={formData.replacement_osteopath_id}
                      onValueChange={(value) => setFormData(prev => ({ ...prev, replacement_osteopath_id: value }))}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Sélectionner un ostéopathe" />
                      </SelectTrigger>
                      <SelectContent>
                        {availableReplacements.map((osteopath) => (
                          <SelectItem key={osteopath.id} value={String(osteopath.id)}>
                            <div className="flex items-center gap-2">
                              {osteopath.name}
                              <Badge variant="outline" className="text-xs">
                                {osteopath.access_type === 'cabinet_colleague' ? 'Collègue' : 'Remplaçant'}
                              </Badge>
                            </div>
                          </SelectItem>
                        ))}
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <Button type="submit">
                      {editingReplacement ? "Mettre à jour" : "Créer"}
                    </Button>
                  </div>
                </form>
              </DialogContent>
          ...
          {availableReplacements.length === 0 && (
            <div className="text-center py-8 bg-muted/50 rounded-lg">
              ...
              <p className="text-muted-foreground">
                Aucun ostéopathe disponible pour les remplacements.
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                Vous devez d'abord être associé à un cabinet ou avoir des collègues.
              </p>
            </div>
          )}
          ...
          {replacements.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">Aucun remplacement configuré</p>
            </div>
          ) : (
                ...
                <div key={replacement.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium">
                        {getOsteopathName(replacement.replacement_osteopath_id)}
                      </span>
                      <Badge variant={replacement.is_active ? "default" : "secondary"}>
                        {replacement.is_active ? "Actif" : "Inactif"}
                      </Badge>
                    </div>
                    {(replacement.start_date || replacement.end_date) && (
                      <p className="text-sm text-muted-foreground">
                        {replacement.start_date && format(new Date(replacement.start_date), "dd/MM/yyyy", { locale: fr })}
                        {replacement.start_date && replacement.end_date && " - "}
                        {replacement.end_date && format(new Date(replacement.end_date), "dd/MM/yyyy", { locale: fr })}
                      </p>
                    )}
                    {replacement.notes && (
                  ...
                  <div className="flex items-center gap-2">
                    <Switch
                      ...
                      onCheckedChange={() => handleToggleActive(replacement.id, replacement.is_active)}
                    />
                    <Button
                      size="sm"
                      variant="outline"
