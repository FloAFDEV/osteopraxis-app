...
export function EnhancedSQLiteDiagnostic() {
  ...
  const runDiagnostic = async () => {
    ...
    try {
      ...
      updateTest(0, { 
        ...
        error: opfsSupported ? undefined : 'OPFS non supporté par ce navigateur',
      ...
      try {
        ...
        updateTest(2, { 
          status: 'error',
          error: error instanceof Error ? error.message : 'Erreur lors de la création des tables',
          duration: Date.now() - startTime3
        });
      }
      ...
      try {
        ...
        const created = await adapters.appointments.create(testAppointment);
        const conflicts = await adapters.appointments.checkConflicts(1, new Date(), 60);
        ...
        updateTest(4, { 
          status: 'error',
          error: error instanceof Error ? error.message : 'Erreur CRUD rendez-vous',
          duration: Date.now() - startTime5
        });
      }
      ...
      try {
        ...
        const monthlyRevenue = await adapters.invoices.getMonthlyRevenue(1);
        ...
        updateTest(5, { 
          status: 'error',
          error: error instanceof Error ? error.message : 'Erreur CRUD factures',
          duration: Date.now() - startTime6
        });
      }
  ...
  const getStatusBadge = (status: DiagnosticTest['status']) => {
    switch (status) {
      ...
      case 'error': return <Badge variant="destructive">Échec</Badge>;
        ...
        <CardContent>
          <div className="space-y-4">
            <Alert>
              ...
              <AlertDescription>
                <strong>Architecture Hybride Phase 1 :</strong> Tests complets de l'infrastructure 
                SQLite avec OPFS pour le stockage local sécurisé des données sensibles.
              </AlertDescription>
            </Alert>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                ...
                <div className="text-2xl font-bold">{globalStats.counts.patients}</div>
                <div className="text-sm text-muted-foreground">Patients</div>
              </div>
              <div className="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                <Calendar className="h-8 w-8 text-green-500 mx-auto mb-2" />
                <div className="text-2xl font-bold">{globalStats.counts.appointments}</div>
              ...
              <div className="text-center p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                ...
                <div className="text-2xl font-bold">{globalStats.counts.invoices}</div>
              ...
              <div className="text-center p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                ...
                <div className="text-2xl font-bold">{globalStats.sizeFormatted}</div>
            ...
            <div className="text-sm space-y-1">
              <div><strong>Version:</strong> {globalStats.version}</div>
              <div><strong>Tables:</strong> {globalStats.tables.join(', ')}</div>
              <div><strong>Support OPFS:</strong> {checkOPFSSupport() ? '✅ Oui' : '❌ Non'}</div>
            </div>
          </CardContent>
        </Card>
      )}
