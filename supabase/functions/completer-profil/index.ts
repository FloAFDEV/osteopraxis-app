...
serve(async (req: Request) => {
  ...
  if (!authHeader) {
    return createErrorResponse('Aucun token d\'authentification fourni', null, 401);
  }
  ...
  try {
    ...
    try {
      ...
      return createErrorResponse('Erreur de lecture du corps de la requête', String(jsonError), 400);
    }
    
    if (!osteopathData) {
      console.error("Pas de données d'ostéopathe fournies");
      return createErrorResponse('Pas de données d\'ostéopathe fournies', null, 400);
    }
    ...
    try {
      ...
      if (existingOsteopath) {
        ...
        if (existingOsteopath.userId !== user.id) {
          return createErrorResponse('Vous ne pouvez pas modifier les données d\'un autre utilisateur', null, 403);
        }
        
        // Mettre à jour l'ostéopathe existant
        const updatedOsteopath = await updateOsteopath(existingOsteopath.id, osteopathData, existingOsteopath);
        result = { osteopath: updatedOsteopath, operation: 'mise à jour', success: true };
      } else {
        // Créer un nouvel ostéopathe
        const newOsteopath = await createOsteopath(user.id, osteopathData);
      ...
      if (result.osteopath && result.osteopath.id) {
        try {
          const userUpdated = await updateUserWithOsteopathId(user.id, result.osteopath.id);
      ...
      console.error("Erreur lors du traitement de la demande:", processingError);
      return createErrorResponse('Erreur lors du traitement', processingError.message);
    }
  } catch (error: any) {
