    ...
    switch (method) {
      case "GET":
        if (consultationId) {
          ...
          const { data, error } = await supabaseClient.from("Consultation").select("*");
          if (error) throw error;
          return new Response(JSON.stringify(data), {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 200,
      ...
      case "PATCH":
        ...
        if (!consultationId) {
          return new Response(
            JSON.stringify({ error: "ID de consultation requis pour la mise à jour" }),
            {
              headers: { ...corsHeaders, "Content-Type": "application/json" },
              status: 400,
            }
      ...
      case "DELETE":
        ...
        if (!consultationId) {
          return new Response(
            JSON.stringify({ error: "ID de consultation requis pour la suppression" }),
            {
              headers: { ...corsHeaders, "Content-Type": "application/json" },
              status: 400,
            }
        ...
        return new Response(JSON.stringify({ success: true, id: consultationId }), {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
        });
