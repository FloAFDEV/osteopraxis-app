    ...
    switch (method) {
      case "GET":
        if (invoiceId) {
          ...
          // Récupérer les factures d'un rendez-vous
          const { data, error } = await supabaseClient
            .from("Invoice")
            .select("*")
      ...
      case "PATCH":
        ...
        if (!invoiceId) {
          return new Response(
            JSON.stringify({ error: "ID de facture requis pour la mise à jour" }),
            {
              headers: { ...corsHeaders, "Content-Type": "application/json" },
              status: 400,
            }
      ...
      case "DELETE":
        ...
        if (!invoiceId) {
          return new Response(
            JSON.stringify({ error: "ID de facture requis pour la suppression" }),
            {
              headers: { ...corsHeaders, "Content-Type": "application/json" },
              status: 400,
            }
