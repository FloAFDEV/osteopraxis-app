-- Fix type mismatches in admin functions

-- Drop and recreate admin_get_system_stats with correct types
...
-- Drop and recreate admin_get_detailed_stats with correct types
...
BEGIN
    ...
    SELECT 
        ...
        -- Chiffre d'affaires
        COALESCE((SELECT SUM(i.amount)::DOUBLE PRECISION FROM "Invoice" i 
                  WHERE i.date >= pr.pr_start AND i.date < pr.pr_end AND i."paymentStatus" = 'PAID' AND i.deleted_at IS NULL), 0.0),
