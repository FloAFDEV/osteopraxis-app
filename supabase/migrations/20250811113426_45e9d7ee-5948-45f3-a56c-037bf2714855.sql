-- Secure RLS for Patient + Audit triggers across sensitive tables
...
-- Allow osteopaths to view their patients or patients in their shared cabinets
CREATE POLICY IF NOT EXISTS "osteopath_read_patients"
ON public."Patient"
FOR SELECT
USING (
...
-- Allow osteopaths to insert patients they own (bind to current osteopath)
CREATE POLICY IF NOT EXISTS "osteopath_insert_patients"
ON public."Patient"
FOR INSERT
WITH CHECK (
  "osteopathId" = public.get_current_osteopath_id()
...
-- Allow osteopaths to update their patients or those in shared cabinets, but ensure owner remains current osteopath
CREATE POLICY IF NOT EXISTS "osteopath_update_patients"
ON public."Patient"
FOR UPDATE
USING (
  ("osteopathId" = public.get_current_osteopath_id())
...
-- Allow osteopaths to delete only their own patients
CREATE POLICY IF NOT EXISTS "osteopath_delete_patients"
ON public."Patient"
FOR DELETE
USING (
  "osteopathId" = public.get_current_osteopath_id()
