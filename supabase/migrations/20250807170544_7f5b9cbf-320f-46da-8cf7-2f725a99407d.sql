...
-- Insérer les plans de base (sans ON CONFLICT pour éviter l'erreur)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.subscription_plans WHERE name = 'Gratuit') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.subscription_plans WHERE name = 'Entreprise') THEN
    INSERT INTO public.subscription_plans (name, price_monthly, price_yearly, max_patients, max_cabinets, features) VALUES
    ('Entreprise', 99.99, 999.99, -1, -1, '{"export": true, "analytics": true, "priority_support": true, "custom_branding": true}');
  END IF;
END $$;

-- Trigger pour mettre à jour updated_at sur subscribers (uniquement si la fonction existe déjà)
DO $$
BEGIN
