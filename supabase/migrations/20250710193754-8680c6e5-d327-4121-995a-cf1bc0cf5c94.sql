-- Corriger les fonctions admin avec les bons types de données

-- 1. Corriger admin_get_system_stats - changer numeric en double precision 
...
RETURNS TABLE(
    ...
    system_revenue double precision,  -- Changed from numeric to double precision
    avg_appointments_per_osteopath double precision,  -- Changed from numeric to double precision
    database_size text
)
LANGUAGE plpgsql
SECURITY DEFINER
...
-- 2. Corriger admin_get_cabinets_with_stats - résoudre l'ambiguïté des colonnes
...
-- 3. Corriger admin_get_orphan_patients - corriger les types timestamp
...
RETURNS TABLE(
    ...
    cabinet_id integer,
    cabinet_name text,
    created_at timestamp with time zone,  -- Changed to timestamp with time zone
    issue_type text
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    SELECT 
        ...
        p."createdAt"::timestamp with time zone as created_at,  -- Cast to timestamp with time zone
